	...
	const int num_views)
{
	...
	if (idx >= num_views * P || !(radii[idx] > 0))
		return;

	// Reading location of 3D covariance for this Gaussian
	const float* cov3D = cov3Ds + 6 * (idx%P);

	// Fetch gradients, recompute 2D covariance and relevant 
	// intermediate forward results needed in the backward.
	float3 mean = means[idx%P];
	float3 dL_dconic = { dL_dconics[4 * idx], dL_dconics[4 * idx + 1], dL_dconics[4 * idx + 3] };
	const float* view_matrix = view_matrix_all + (idx/P)*4*4;
	...
	if(antialiasing)
	{
		...
		const float d_h_convolution_scaling = dL_dopacity_v * opacities[idx%P];
	...
	const int num_views)
{
	...
	float3 m = means[idx%P];

	const float* proj = proj_matrix + (idx/P) * 4 * 4;
	...
	atomicAdd(&dL_dmeans[idx%P].x, dL_dmean.x);
	atomicAdd(&dL_dmeans[idx%P].y, dL_dmean.y);
	atomicAdd(&dL_dmeans[idx%P].z, dL_dmean.z);
	...
	if (scales)
		computeCov3D(idx, scales[idx%P], scale_modifier, rotations[idx%P], dL_dcov3D, dL_dscale, dL_drot);
}
...
)
{
	...
	uint32_t view_id = block.group_index().x / horizontal_blocks;
	const uint2 pix_min = { (block.group_index().x % horizontal_blocks) * BLOCK_X, block.group_index().y * BLOCK_Y };
	...
	const uint2 range = ranges[view_id * tile_grid.x * tile_grid.y + block.group_index().y * horizontal_blocks + (block.group_index().x % horizontal_blocks)];
	...
	const float T_final = inside ? final_Ts[view_id * W * H + pix_id] : 0;
	float T = T_final;

	// We start from the back. The ID of the last contributing
	// Gaussian is known from each pixel from the forward.
	uint32_t contributor = toDo;
	const int last_contributor = inside ? n_contrib[view_id * W * H + pix_id] : 0;
	...
	{
		for (int i = 0; i < C; i++)
			dL_dpixel[i] = dL_dpixels[view_id*C*H*W + i * H * W + pix_id];
		if(dL_invdepths)
		dL_invdepth = dL_invdepths[view_id * H * W  + pix_id];
	}
	...
	for(int j=0;j<=view_id;j++) num_rendered_before += num_rendered_per_view[view_id];
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		{
			const int coll_id = point_list[num_rendered_before + range.y - progress - 1];
			collected_id[block.thread_rank()] = coll_id;
			collected_xy[block.thread_rank()] = points_xy_image[P * view_id + coll_id];
			collected_conic_opacity[block.thread_rank()] = conic_opacity[P * view_id + coll_id];
			for (int i = 0; i < C; i++)
				collected_colors[i * BLOCK_SIZE + block.thread_rank()] = colors[(coll_id%P) * C + i];

			if(dL_invdepths)
			collected_depths[block.thread_rank()] = depths[P * view_id + coll_id];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				atomicAdd(&(dL_dcolors[P*view_id + global_id * C + ch]), dchannel_dcolor * dL_dchannel);
			}
			...
			atomicAdd(&(dL_dinvdepths[P*view_id + global_id]), dchannel_dcolor * dL_invdepth);
			}
			...
			atomicAdd(&dL_dmean2D[P*view_id + global_id].x, dL_dG * dG_ddelx * ddelx_dx);
			atomicAdd(&dL_dmean2D[P*view_id + global_id].y, dL_dG * dG_ddely * ddely_dy);

			// Update gradients w.r.t. 2D covariance (2x2 matrix, symmetric)
			atomicAdd(&dL_dconic2D[P*view_id + global_id].x, -0.5f * gdx * d.x * dL_dG);
			atomicAdd(&dL_dconic2D[P*view_id + global_id].y, -0.5f * gdx * d.y * dL_dG);
			atomicAdd(&dL_dconic2D[P*view_id + global_id].w, -0.5f * gdy * d.y * dL_dG);

			// Update gradients w.r.t. opacity of the Gaussian
			atomicAdd(&(dL_dopacity[P*view_id + global_id]), G * dL_dalpha);
		}
	}
}
	...
	int num_views)
{
	...
	computeCov2DCUDA << <(num_views*P + 255) / 256, 256 >> > (
		P,
		means3D,
		radii,
		cov3Ds,
	...
	preprocessCUDA<NUM_CHANNELS> << < (num_views*P + 255) / 256, 256 >> > (
		P, D, M,
		(float3*)means3D,
		radii,
		shs,
...
{
	dim3 new_grid;
	new_grid.x = grid.x * num_views;
	new_grid.y = grid.y;
	new_grid.z = grid.z;
	renderCUDA<NUM_CHANNELS> << <new_grid, block >> >(
		ranges,
		point_list,
		W, H,
		bg_color,
